...
def show_releases():
    ...
    if session['tune'] is not None:
        query = session['tune']
        cur.execute("SELECT release_string FROM tu_release r "
        ...
        cur.execute("SELECT release_string FROM tu_release "
                    "WHERE release_string LIKE %s", 
                    ('%' + band_with_dashes + '%',))
    elif session['part'] is not None:
        query = session['part']
        part_with_dashes = session['part'].replace(' ', '-')
        cur.execute("SELECT release_string, s.song_title FROM tu_release r "
                    "JOIN tu_song_release sr ON r.release_id = sr.release_id "
